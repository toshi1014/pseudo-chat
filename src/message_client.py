    ...
    def on_message(self, ws, message_in):
        ...
        if message_in[:4] == "[js]":
            if message_in[:8] == "[js]send":
                message_out = generate_message.GenerateMessage(self.opponent_name).get_generated_message("me")
            elif message_in[:11] == "[js]receive":
                message_out = generate_message.GenerateMessage(self.opponent_name).get_generated_message(self.opponent_name)
